...
const PAGINATION_TEMPLATE = `
  <ul class="pagination" [ngClass]="classMap" role="navigation" aria-label="Pagination">
    <li *ngIf="boundaryLinks"
        class="pagination-previous"
        [class.unavailable]="noPrevious()||disabled">
      <a href (click)="selectPage(1, $event)" [innerHTML]="getText('first')"></a>
    </li>
    <li *ngIf="directionLinks"
        [class.pagination-previous]="!boundaryLinks"
        [class.unavailable]="noPrevious()||disabled">
      <a href (click)="selectPage(page - 1, $event)" [innerHTML]="getText('previous')"></a>
    </li>
    <li *ngFor="let pg of pages; let isFirst=first; let isLast=last"
        [class.current]="pg.active"
        [class.unavailable]="disabled && !pg.active"
        [class.pagination-previous]="isFirst && !directionLinks && !boundaryLinks"
        [class.pagination-next]="isLast && !directionLinks && !boundaryLinks"
        class="pagination-page">
      <a *ngIf="!pg.active" href (click)="selectPage(pg.number, $event)" [innerHTML]="pg.text"></a>
      <span *ngIf="pg.active">{{pg.text}}</span>
    </li>
    <li *ngIf="directionLinks"
        [class.pagination-next]="!boundaryLinks"
        [class.unavailable]="noNext()">
      <a href (click)="selectPage(page + 1, $event)" [innerHTML]="getText('next')">
      </a>
    </li>
    <li *ngIf="boundaryLinks"
        class="pagination-next"
        [class.unavailable]="noNext()">
      <a href (click)="selectPage(totalPages, $event)" [innerHTML]="getText('last')">
      </a>
    </li>
  </ul>
  `;
